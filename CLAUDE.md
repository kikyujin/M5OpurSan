...
### 焼くときは**本体スイッチを OFF** にして繋ぐ

スリープを入れてから、焼く前に守ることが 3 つ増えた（2026-08-16 に全部踏んだ）。

1. **本体スイッチ OFF で USB に繋ぐ。** そうすると USB が唯一の電源になり、
   `getBatteryLevel()` が必ず 100% を返す。アプリは 100% を「給電中」と見て
   **スリープしない**ので、焼いている最中に寝られる事故が起きない。
   スイッチ ON だとバッテリーの実残量が出るため、充電中でも 100% には
   届かず、普通に寝てしまう
2. **焼く前に短い操作で通信を確かめる**（`esptool.py ... flash_id`）。
   **`/dev/cu.usbmodem*` があることは生きている証拠にならない。**
   ディープスリープで USB CDC が切れてもノードだけ残ることがあり、
   それを見て「起きている」と誤判断して焼きにいくと、途中で切れて
   **アプリ領域が半端な状態になり起動しなくなる**
3. 寝ている実機に esptool を当てない。マスターにキーを押して起こしてもらう

### 長い転送が途中で落ちるときは分割して焼く

USB が詰まると、短い操作は通るのに 1.6MB の一括書き込みだけが落ちる、という
状態になることがある（`Could not configure port: (6, 'Device not configured')`）。
**不良セクタと紛らわしいが、その番地を直接 16KB 読めるなら Flash は無事。**

64KB ずつに割って `write_flash` を繰り返せば通る。全部書けて正常起動すれば、
そのあとは一括書き込みも元どおり通るようになる。
...
ESC メニューの **`5`**。32 行のリングバッファ。**4 は送信**なので押し間違えないこと
（`5` はメニューに表示していない。開発用なので常用の導線に置いていない）。
...
**読む前に、そのコアダンプが今回のものか確かめること。** 消えずに残るので、
何ヶ月も前のクラッシュを掘り当てていることがある（2026-08-16 に、8/15 の
NVS パニックを今日の障害だと思って追いかけた）。照合はこう:

```bash
strings core.elf | grep -i "assert\|panic"   # パニック理由
xtensa-esp32s3-elf-readelf -n core.elf       # ESP_CORE_DUMP_INFO に app SHA
shasum -a 256 firmware.elf | cut -c1-16      # 焼いたものの SHA と突き合わせる
```

**WDT リセットはコアダンプを残さないことがある**（パニックハンドラを
通らないため）。そのときはパンくずしか手掛かりが無いので、
新しい経路を足したら**必ずパンくずも置く**。ひとつの番号が広い範囲を
指していると場所が絞れない（21「キー待ち」が `getch()` と無操作処理の
両方を指していて、1 往復無駄にした）。
