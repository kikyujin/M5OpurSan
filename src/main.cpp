...
static void light_sleep_cycle(void) {
    ...
    //
    // もとは「ライトスリープ中に充電されない」疑いを確かめるために足した行。
    // 再現しなかったので疑い自体は晴れたが（opur_sleep.cpp の注記）、
    // **同じ疑いが再燃したときに一番先に見る材料なので残してある。**
    //
    // **画面と WiFi を戻す前に読む。** ここが寝ていたときの状態にいちばん近い。
    // あとに回すとバックライトと WiFi の負荷が乗る。
    // 「低負荷だから充電が止まった」たぐいの疑いを見るときに、
    // 負荷が戻った瞬間の跳ね上がりを拾ってしまうと判別できなくなる。
    //
    // mV は毎回 ADC を読む。% のほうは 30 秒キャッシュだが、
    // 寝ていた時間がそれを超えていれば読み直される（m5curses.cpp の注記）。
    //
    // **ログは 32 行のリング。**入眠と合わせて 1 周 2 行使うので履歴は半分になる。
    // 前後比較を 1 箇所で見られるほうが得だと判断した。
