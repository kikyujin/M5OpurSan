...
| 経過 | 状態 | 起こしかた |
|--|--|--|
| 1 分 | 画面が消える（ライトスリープ） | 何かキーを押す。書きかけはそのまま残っている |
| さらに 10 分 | 書きかけを `AUTOSAVE.txt` に逃がしてディープスリープ | キーを押すか、2 分後にひとりでに起きる |

ディープスリープから戻ると起動しなおしになるが、`AUTOSAVE.txt` があれば**書きかけの続きから**始まる。読み込んだ時点でこのファイルは消えるので、二重に復元されることはない。復元した本文は「まだ保存していない」扱いで、保存すると新しい番号が付く。
