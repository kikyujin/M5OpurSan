...
// **検証ブランチ（verify-charger-autooff）では 1 に戻してある。**
// **main では 0（ペンディング中）。このブランチを main にマージしないこと。**
//
// ライトスリープ中、USB を挿していてもバッテリーが充電されないことが実機で
// 分かった（2026-08-17）。main では原因が確定するまで丸ごと無効にしてある。
// 調査結果は ~/claude-store/orders/done/
// 2026-08-17-001_m5unified-charge-gpio-investigation_result.md。
//
// その後の実測で分かったこと:
//   - Active（スイッチ ON・Anker 充電器）では 37% → 57% まで回復する。
//     **Active では充電が勝つ**ので「消費が充電を上回っている」説は棄却
//   - ライトスリープの消費は Active より圧倒的に小さいのに充電されない
//   - シルクの CN809 横のダイオードが 5VIN → BAT_OUT の直接給電経路。
//     **充電と負荷は食い合わない**ので、TP4057 が充電を止める理由が無い
//
// 残る犯人は **5VIN を消せる者だけ ＝ 充電器側**。Anker には低負荷で VBUS を
// 落とすオートオフを持つモデルが多い（仮説 F）。
//   Active     100mA 超  → 充電器は生きたまま → 充電される
//   Light Sleep 数 mA    → 「未接続」と判断されて VBUS 遮断 → TP4057 の VIN 消失
//
// **このブランチはそれを実機で確かめるためだけのもの。** 寝かせて放置し、
// 起こして入眠/起床の mV を比べる。手順は
// ~/claude-store/orders/done/2026-08-17-003_..._result.md。
//
