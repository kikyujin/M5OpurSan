...
// ライトスリープの有効・無効。**いまは 1（有効）。**
//
// 2026-08-17 に「ライトスリープ中、USB を挿していても充電されない」という
// 観測があり、一度これを 0 にして丸ごと止めた。**その後の実機テストで
// 再現しなかったので戻した。** ライトスリープは充電と共存できる。
//
// 実測（スイッチ ON・Anker 充電器）:
//   Active                        37% → 57%          増
//   Light Sleep（挿しっぱなし）   44%(3656mV) → 54%(3734mV)  増
//   Light Sleep（寝てから挿す）   → 60%              増
//
// **当初の観測が何だったかは分かっていない。** 潰した仮説は残しておく:
//   - 消費が充電電流を上回っていた → 棄却（Light Sleep のほうが消費は小さい）
//   - 充電器のオートオフ（低負荷で VBUS 遮断）→ 棄却（寝ていても充電された）
//   - 挿すタイミング → 棄却（寝てから挿しても充電された）
// 調査の経緯は ~/claude-store/orders/done/ の 2026-08-17-001〜004。
// **同じ症状が出たら、まずそこを読んでから疑うこと。**
//
// なお ESP32 側にライトスリープで充電を切る要素は無い（001 で確認済み）:
// M5Unified は ADV の充電パスに一切触らず、Cardputer には POWER_HOLD も無い。
// ライトスリープは digital 側のパッド状態を保持するので、
// 仮に充電イネーブルが GPIO で出ていても落ちない。
//
// これを 0 にするときは、寝る前の画面 OFF / WiFi 切断が呼び出し側の担当な点に
// 注意（src/main.cpp の idle_check を参照）。片方だけ止めると、無操作のあいだ
// 「消して点けて、切って繋いで」を繰り返すことになる。
