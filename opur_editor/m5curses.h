...
// USB CDC が張れていれば 1。
//
// **「USB が挿さっているか」ではない。** ホストがポートを開いて実際に読み書き
// している状態でしか 1 にならない（HWCDC::isCDC_Connected() は、ホストが
// TX FIFO を引き取ったときに立つフラグを見ている）。PC に挿しただけで
// ターミナルを開いていなければ 0 のまま——実機で確認済み。
//
// したがって**給電の判定には使えない。** 残っているのは診断用（ログに出して
// 「そのとき CDC が生きていたか」を後から見る）と、たまたま 1 のときに
// スリープを止める保険としてだけ。
//
// 給電中に寝ないようにする本命は「電池 100%」のほうで、それが当てになるのは
// **本体スイッチ OFF のとき**（USB が唯一の電源になるので必ず 100% を指す）。
// 焼くときはスイッチ OFF で繋ぐ、という運用で担保している（CLAUDE.md 参照）。
